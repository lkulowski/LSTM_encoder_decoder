...
That is, we recurrently feed the predicted decoder outputs, <a href="https://www.codecogs.com/eqnedit.php?latex=\boldsymbol{\hat{y}}^{(\tau_{i}&space;)}" target="_blank"><img src="https://latex.codecogs.com/gif.latex?\boldsymbol{\hat{y}}^{(\tau_{i}&space;)}" title="\boldsymbol{\hat{y}}^{(\tau_{i} )}" /></a>, into the LSTM decoder until we have an output of the desired length. Second, we can make predictions using teacher forcing. In teacher forcing, we feed the true target value into the LSTM decoder, as shown below. 
