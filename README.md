...
For the LSTM encoder-decoder, we need to organize our data into sequences of _n<sub>i</sub>_ input values and _n<sub>o</sub>_ target values. To do this, we slide a moving window over the dataset. We start at the first _y_ value and collect _n<sub>i</sub>_ values as input and the next _n<sub>o</sub>_ values as targets. Then, we slide our window to the second (stride = 1) or third (stride = 2) _y_ value and repeat the procedure. The windowed dataset for _n<sub>i</sub>_ = 3, _n<sub>o</sub>_ = 2, and stride = 1 is shown below. 
